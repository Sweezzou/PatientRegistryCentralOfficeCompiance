...
export const EditPatientForm = ({ selectedPatId, openEditModal, closeEditModal }) => {
    ...
    const fetchPatientById = async (id) => {
        try {
            const response = await PatientService.getPatientById(id, token);
            const {
                lastName,
                firstName,
                middleName,
            ...
            const imageUrl = photoUrl ? await PatientService.getPatientPhoto(photoUrl, token) : '';
            ...
            setUpdatedPatientData({
                ...
                phoneNo: phoneNo || '',
                photoUrl: photoUrl || '',
                patientStatus: patientStatus || false,
    ...
    useEffect(() => {
        if (openEditModal && selectedPatId) {
            fetchPatientById(selectedPatId);
        }
    }, [selectedPatId, openEditModal]);
    ...
    const handleEditInputChange = (e) => {
        ...
        if (name === "photo") {
            ...
            setUpdatedPatientData(prevPatientData => {
                return {
                    ...prevPatientData,
                    [name]: formattedValue
                };
    ...
    const handleSubmitEdit = async (e) => {
        ...
        if (!updatedPatientData.lastName.trim()) { validationErrors.lastName = " *"; }
        if (!updatedPatientData.firstName.trim()) { validationErrors.firstName = " *"; }
        if (!updatedPatientData.middleName.trim()) { validationErrors.middleName = " *"; }
        if (!updatedPatientData.sex.trim()) { validationErrors.sex = " *"; }
        if (!updatedPatientData.placeOfBirth.trim()) { validationErrors.placeOfBirth = " *"; }
        if (!updatedPatientData.birthdate.trim()) { validationErrors.birthdate = " *"; }
        if (!updatedPatientData.address.trim()) { validationErrors.address = " *"; }
        if (!updatedPatientData.phoneNo.trim()) { validationErrors.phoneNo = " *"; }
        ...
        try {
            ...
            updateFormData.append("patient", JSON.stringify(updatedPatientData));
            setIsConfirmUpdate(true);
        } catch (error) {
            console.error("Error checking Patinet ID:", error);
            toastError("Error checking Patient ID: " + error.message);
        }
    };
    ...
    const handleConfirmUpdate = async () => {
        try {
            ...
            const response = await PatientService.updatePatient(selectedPatId, updateFormData, token);
        ...
        <div id="addModal" className="fixed inset-0 z-50 overflow-x-auto bg-gray-900 bg-opacity-50 px-2 py-2 mx-auto md:h-screen lg:py-0">
            <div className="relative lg:m-auto lg:my-8 sm:m-4 w-full max-w-5xl p-4 bg-gray-50 rounded-xl shadow-xl">
                <div className="mb-3 flex items-center justify-between bg-green-800 rounded-lg shadow-gray-500 shadow-md h-9">
                    <h3 className="text-start text-base text-gray-50 font-semibold flex-grow px-3">Edit Patient</h3>
                    <button onClick={closeEditModal} type="button" className="text-gray-400 hover:text-gray-500 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-hide="addModal">
                ...
                <form onSubmit={handleSubmitEdit}>
                    <div className="grid place-items-center grid-cols-1 mb-2">
                        <div className="relative flex bg-gray-200 rounded-3xl shadow-lg shadow-gray-300 h-60 max-w-60 w-full justify-center items-center hover:cursor-pointer hover:ring-4 hover:ring-gray-200" onClick={handleFileInputClick}>
                            {photoPreview ? (
                                <img src={photoPreview} alt="photoPreview" className="w-full h-full object-cover rounded-3xl" />
                            ) : (
                            ...
                            <input type="file" accept="image/*" ref={fileInputRef} onChange={handleEditInputChange} name="photo" className="hidden" />
                            {photoPreview && (
                    ...
                    <div className="mb-2 grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="mb-1">
                            ...
                            <input type="text" value={updatedPatientData.lastName} onChange={handleEditInputChange} id="lastName" name="lastName"
                        ...
                        <div className="mb-1">
                            ...
                            <input type="text" value={updatedPatientData.firstName} onChange={handleEditInputChange} id="firstName" name="firstName"
                                className={`bg-gray-50 border border-gray-300 ${errors.firstName ? 'border-red-500 outline outline-offset-0 outline-red-200' : 'border-gray-200'} text-gray-600 sm:text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full py-1 px-2.5`} />
                        </div>
                        <div className="mb-1">
                            ...
                            <input type="text" value={updatedPatientData.middleName} onChange={handleEditInputChange} id="middleName" name="middleName"
                                className={`bg-gray-50 border border-gray-300 ${errors.middleName ? 'border-red-500 outline outline-offset-0 outline-red-200' : 'border-gray-200'} text-gray-600 sm:text-sm rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full py-1 px-2.5`} />
                        </div>
                        <div className="mb-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <SelectSuffix value={updatedPatientData.suffix} onChange={handleEditInputChange} />
                            </div>
                            <div>
                                ...
                                <SelectSex value={updatedPatientData.sex} onChange={handleEditInputChange} error={errors.sex} />
                            </div>
                        </div>
                    </div>
                    <div className="mb-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="mb-1">
                            ...
                            <SelectCivilStatus value={updatedPatientData.civilStatus} onChange={handleEditInputChange} error={errors.civilStatus} />
                        </div>
                        <div className="mb-1 md:col-span-2">
                            ...
                            <input type="text" value={updatedPatientData.placeOfBirth} onChange={handleEditInputChange} id="placeOfBirth" name="placeOfBirth" autoComplete="off"
                    ...
                    <div className="mb-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="mb-1">
                            ...
                            <input type="date" value={updatedPatientData.birthdate} onChange={handleEditInputChange} id="birthdate" name="birthdate" autoComplete="off"
                        ...
                        <div className="mb-1 md:col-span-2">
                            ...
                            <input type="text" value={updatedPatientData.address} onChange={handleEditInputChange} id="address" name="address" autoComplete="off"
                    ...
                    <div className="mb-2 grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="mb-1">
                            ...
                            <SelectBloodType value={updatedPatientData.bloodType} onChange={handleEditInputChange} error={errors.bloodType} />
                        </div>
                        <div className="mb-1">
                            ...
                            <input type="text" value={updatedPatientData.phoneNo} onChange={handleEditInputChange} id="phoneNo" name="phoneNo" autoComplete="off"
                    ...
                    <div className="flex items-center justify-start mt-6 space-x-5">
                        <button type="submit" className="bg-green-700 hover:bg-green-800 text-white font-semibold rounded-lg focus:outline-none focus:shadow-outline w-28 h-8">Update</button>
                        <button type="button" onClick={closeEditModal} className="bg-gray-500 hover:bg-gray-700 text-white font-semibold rounded-lg focus:outline-none focus:shadow-outline w-28 h-8">Cancel</button>
                    </div>
                </form>
                <ConfirmUpdate closeModal={closeConfirmUpdate} openModal={isConfirmUpdate} onConfirm={handleConfirmUpdate} />
            </div>
        </div>
    )
}
